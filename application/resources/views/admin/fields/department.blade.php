...
@section('meta')
    <title>Nova Turma | Webponto</title>
...
<div class="container-fluid">
    <div class="row">
        <div class="col-md-12">
            <h2 class="page-title uppercase">{{ __("Adicionar Turma") }}
          <!--  <button class="ui basic button mini offsettop5 btn-import float-right"><i class="ui icon upload"></i> {{ __("Import") }}</button>
            <a href="{{ url('export/fields/department' )}}" class="ui basic button mini offsettop5 btn-export float-right"><i class="ui icon download"></i> {{ __("Export") }}</a> -->
            </h2>
        </div>    
    </div>
            ...
            <div class="box box-success">
                <div class="box-body">
                    ...
                    <form id="add_department_form" action="{{ url('fields/department/add') }}" class="ui form" method="post" accept-charset="utf-8">
                        @csrf
                        <div class="field">
                            <label>{{ __("Turma") }} <span class="help">Exemplo: 2024.1 - CMT Jovem - Quarta-feira</span></label>
            ...
            <table width="100%" class="table table-striped table-hover" id="dataTables-example">
                ...
                <tbody>
                    @isset($data)
                        ...
                        <tr><td>{{ $department->id_turma }}</td>
                            <td>{{ $department->department }}</td>
                            <td>{{ $department->semestre }}</td>
